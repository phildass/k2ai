...
## Predetermined FAQ Answers

The chatbot includes a **FAQ matching system** that checks for predetermined answers **before** calling the OpenAI API. This provides:
- **Faster responses** for common questions
- **Consistent answers** for important topics like fees, contact info, etc.
- **Cost savings** by reducing API calls

### How It Works

1. User sends a message to `/api/chat/`
2. System checks if the message matches any FAQ keywords
3. If a match is found → return predetermined answer immediately
4. If no match → fallback to OpenAI for a dynamic AI response

### FAQ Configuration

FAQs are stored in `backend/private_faq/faqs.json`:
...
```json
{
  ...
  "contact": {
    ...
    "answer": "You can reach us at enquiry@k2communications.in..."
  }
}
```

### Updating FAQs (Admin Only)

1. Edit `backend/private_faq/faqs.json`
2. Add/modify FAQ entries
3. Restart the server to reload FAQs

See [private_faq/README.md](private_faq/README.md) for detailed instructions.

### Response Format

The API response includes an `answer_source` field:
...
```json
{
  ...
  "suggestions": [...],
  "metadata": {...},
  "answer_source": "faq_match"  // or "ai"
}
```

- `"faq_match"` - Answer came from predetermined FAQs
- `"ai"` - Answer generated by OpenAI
...
# Response will have "answer_source": "faq_match"

# Test non-FAQ question (falls back to AI)
curl -X POST http://localhost:8000/api/chat/ \
  -H "Content-Type: application/json" \
  -d '{"message": "What is public relations?"}'

# Response will have "answer_source": "ai"
```
...
│   └── faq_service.py      # FAQ matching service
├── models/              # Data models
│   └── schemas.py      # Pydantic models
├── private_faq/         # Predetermined FAQ answers (admin only)
│   ├── faqs.json       # FAQ data (keywords and answers)
│   └── README.md       # FAQ management guide
